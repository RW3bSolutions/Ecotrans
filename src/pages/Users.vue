    ...
    <q-card style="width: 700px; max-width: 80vw;">
      <form @submit.prevent="ifEdit ? update() : store()">
        ...
        <q-card-section>
          ...
          <div>
            <q-select filled color="dark" label="Role *" square v-model="form.role" :dense="true" :options="['Admin','HR','Account']" />
...
export default defineComponent({
  ...
  methods: {
    ...
    update () {
      this.submitted = true
      this.errors.show = false
      this.$api.post('system/users/' + this.object.id, {
        name: this.form.name,
        username: this.form.username,
        role: this.form.role,
        is_active: this.form.is_active,
